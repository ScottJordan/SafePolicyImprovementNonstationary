...
class RandomScenario(Scenario):
    def __init__(self, start_time=None, seed=None):
        ...
        super(RandomScenario, self).__init__(start_time=start_time)
        ...
        for p, tlb, tub, tbar, tsd, mbar, msd in zip(
            ...
            if self._rng.rand() < p:
                ...
                scenario['meal']['amount'].append(
                    max(round(self._rng.normal(mbar, msd)), 0))
    ...
    def reset(self):
        ...
        # Could keep changing the randomstate
        # But simpler alternative is not reset the seed
        # self._rng = np.random.RandomState(self._rng.randint(2**31))
...
class WeightScenario(Scenario):
    # Reference for this scenario
    # https://openreview.net/pdf?id=ByexVzSAs4

    def __init__(self, weight, start_time=None, seed=None):
        # Scenario.__init__(self, start_time=start_time)
        super(WeightScenario, self).__init__(start_time=start_time)
        self.weight = weight
        self.seed = seed
        self._rng = np.random.RandomState(seed)
    ...
    def create_scenario(self):
        ...
        time_mu = np.array([7, 9.5, 12, 15, 18, 21.5]) * 60
        time_sigma = np.array([60, 30, 60, 30, 60, 30])
        amount_mu = np.array([0.7, 0.15, 1.1, 0.15, 1.25, 0.15]) * self.weight
